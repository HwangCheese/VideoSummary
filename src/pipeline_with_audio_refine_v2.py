...
from audio_refinement import get_db_levels, adjust_segments_with_audio
from resolve_overlap import resolve_overlap
...
def run_pipeline(video_path, ckpt_path, output_dir, device="cpu", fps=1.0, alpha=0.7, std_weight=0.3, top_ratio=0.2):
    ...
    audio_wav_path = os.path.join(output_dir, "temp.wav")
    ...
    print("\n🎧 [2.5/3] 데시벨 기반 세그먼트 조정", flush=True)
    subprocess.run([
    ...
    db_levels = get_db_levels(audio_wav_path, frame_ms=100)
    selected_segments = adjust_segments_with_audio(selected_segments, db_levels, fps=fps)
    selected_segments = resolve_overlap(selected_segments)  # ✅ 겹침 방지 적용
