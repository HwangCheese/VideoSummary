...
def run_importance_calculation(ckpt_path, feature_h5, device):
    """
...
#     # **V1 변경점:** V1 모델은 forward에 'mask' 인수를 받지 않습니다.
#     # V1 모델은 forward에 'seq_len' 인수를 받습니다.
#     # V1 모델은 시퀀스 길이가 필요하며, 특징 텐서 x의 두 번째 차원입니다.
#     seq_len = x.shape[1]
    
#     # 3. 모델 추론(Inference) 실행 
#     with torch.no_grad():
#         # **V1 forward 호출: scores, att_weights = model(x, seq_len)**
#         # V1 모델은 시퀀스 길이를 받으므로, seq_len을 전달해야 합니다.
#         scores, att_weights = model(x, seq_len) 
...
#     # **V1 변경점 1:** VASNet.__init__이 hidden_dim 인수를 받지 않습니다.
#     model = VASNet() 
